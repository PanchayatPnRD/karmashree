...
const Accidentlist = ({ refreshList, setShouldRefresh }) => {
  ...
  const handleSearchHcf = () => {
    getAllAccidentList(fromDate, endDate);
  }


  const getAllAccidentList = () => {
   accidentApi.callListAccident({
      page: currentPage,
      limit: perPage,
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-blue-50 text-left dark:bg-meta-4 ">
                  {["Sl. No.", "Events", "Assessment", "Measures Taken", "Waste Involved","Steps to Elavate", "Steps to Prevent"].map((el) => (
                    <th
                      key={el}
                    ...
                    <tr key={key}>
                      ...
                      <td>
                       ...
                       <td>
                        <div className={className}>
                          {item?.waste_involved ? (
                            item.waste_involved.split(",").map(d => (
                             <span
                              key={d}
                              className={`me-1 px-2 py-1 rounded text-black ${getBorderColorById(parseInt(d))} ${getTextColorById(parseInt(d))} ${getColorById(parseInt(d))}`}
                              style={{ border: "1px solid black" }} 
                             >
                          {getColorNameByCode(parseInt(d))}
                         </span>
                         ))
                       ) : (
                            <span className="px-2 py-1 rounded text-black" style={{ border: "1px solid black" }}>
                              N/A
                           </span>
                       )}
                        </div>
                      ...
                      <td>
                        <div className={className}>
                          {item['steps_to_alleviate']}
                        </div>
                      </td>
                      <td>
                        <div className={className}>
                          {item['steps_to_prevent']}
                        </div>
                      </td>
