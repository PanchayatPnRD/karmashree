...
const Vaccinelist = ({ refreshList, setShouldRefresh }) => {
  ...
  const handleSearchHcf = () => {
    getAllVaccineList(keyword,fromDate, endDate);
  }


  const getAllVaccineList = () => {
    vaccineApi.callListVaccine({
      page: currentPage,
      limit: perPage,
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-blue-50 text-left dark:bg-meta-4 ">
                  {["Sl. No.","No. of Employees", "Vaccine Type","Vaccination Date"].map((el) => (
                    <th
                      key={el}
                ...
                {data && data?.data?.map((item, key) => {
                  ...
                  return (
                    <tr key={key}>
                      ...
                      <td>
                        <div className={className}>
                         {item?.vaccinationType?.vc_name}
                        </div>
                      </td>

                      <td>
                        <div className={className}>
                         {moment(item['vaccination_date']).format("Do MMM, YYYY")}
                        </div>
                      </td>
