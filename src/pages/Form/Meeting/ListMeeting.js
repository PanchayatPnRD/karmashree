...
const Meetinglist = ({ refreshList, setShouldRefresh }) => {
  ...
  const handleSearchHcf = () => {
    getAllMeetingList(fromDate, endDate);
  }


  const getAllMeetingList = () => {
    meetingApi.callMeetList({
      page: currentPage,
      limit: perPage,
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-blue-50 text-left dark:bg-meta-4 ">
                  {["Sl. No.", "Meet Title", "Meeting Schedule", "Meeting Agenda", "Meeting Type", "Meeting Organiser"].map((el) => (
                    <th
                      key={el}
 ...
 */}
      ...
      {/* <UpdateTreatmentModal open={updateEquip != null} handleOpen={() => setUpdateEquip(null)} data={updateEquip} setShouldRefresh={setShouldRefresh} /> */}
